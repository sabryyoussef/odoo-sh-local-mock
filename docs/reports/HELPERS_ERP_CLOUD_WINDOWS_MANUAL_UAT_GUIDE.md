...
## V3 Updates (2026-09-06T06:50Z, run 20260906T040329Z_55e870ce) — FINAL Handoff (4 accounts, 0 tenants)

- **Credential rotation (2x):** first leak SESSION_SECRET (hash 03e278f74b4737a5 len 32) rotated 2026-09-06T07:04Z to 43-char token_urlsafe(32)[:43] (hash 549c59a1b598a7c7); second leak docker inspect printed BUILD_POSTGRES_ADMIN_PASSWORD/BUILD_POSTGRES_PASSWORD (hashes dc468b376cdbcbbb/56acc751d884d644 len 43) into Roo stdout cmd-1788674690340.txt — sanitized to ***REDACTED*** and rotated again 2026-09-06T09:13Z to new 43-char (hashes 8854bf0272a3a83a/76a4556c5005e2fb). Verified new succeeds, old/leaked/previous-new/wrong fail under scram-sha-256 (7 scram, 0 trust), pg_hba hardened, Postgres internal only PortBindings {}, .env 600, odoo.conf updated, p3-uat-control-api restarted via docker compose -f docker-compose.uat.yml up -d --force-recreate, portal 200.
- **Installer fail-closed:** control-api/app/services/cloud_manual_uat_provisioner.py fixed detach=False (returns bytes) to detach=True + container.wait(timeout=600) with redacted logs, verification SELECT name FROM ir_module_module WHERE state='installed' must contain all to_install else RuntimeError, outer provision_manual_uat_request calls without try/except so failure rolls back tenant (never marks ready if modules missing). Commit 96f0274.
- **Translations fix:** control-api/app/translations.py products.card3.title Ship custom ERP from Git to Ship Custom Odoo from Git (byte-identical to test_three_product_navigation assertion), verified curl http://100.76.217.35:8001/ and pytest 6 passed. Commit 1b14ff3.
- **Accounts-only seed (fixes pre-queue violation):** new seed_manual_uat_accounts_only() and --prepare-manual-accounts-only (commit 2ea8822) — creates only users+setups (4 accounts, 0 tenants), leaves Create click to queue provisioning. READY forbids pre-created tenants; V2 --prepare-manual pre-queued (violation) — V3 FINAL uses accounts-only.
- **Isolated worker:** new control-api/app/manual_uat_worker_main.py (commit 541e848, 267 lines) — allow-list helpers_demo_user1-4/user1-4/requests 4-7, requires HELPERS_CLOUD_MANUAL_UAT_ENABLED=true and APP_ENV != production, bounded --max-success 4 one at a time, heartbeat /data/manual_uat_worker_heartbeat.json, never touches live data/control.db or live provisioning-worker.
- **Playwright recapture:** BASE http://100.76.217.35:8001 (not 127.0.0.1), 44 PNGs desktop 1280x800 + Pixel 5 mobile (pricing monthly/annual, login EN/AR, per-user instances/setup/pricing/instance-detail/review/confirm, Odoo 8301-8304), verified http://100.76.217.35:830N in portal HTML, no 127.0.0.1 in external URLs.
- **Full tests:** 35 files, 426 passed, 0 failed, 13 skipped (batch1 61, batch2 164+10 skipped, batch3 201+3 skipped), SKIP_DOCKER_SCAN=1 for docker-dependent, batched due to 300s CLI timeout, manifest phase10/test_manifest.json.
- **Final manual state (Phase 12):** docker exec p3-uat-control-api python -m app.scripts.seed_helpers_cloud_manual_uat --reset deleted exact 4 identities (users 11-14, subs 4-7, requests 4-7, instances 4-7, tenants 27-30, DBs, roles, containers, filestores), then --prepare-manual-accounts-only recreated 4 accounts 0 tenants (user1 trial sales User 1 Demo Company setup 4 draft, user2 starter trading setup 5, user3 business operations setup 6, user4 enterprise full_erp setup 7). DB counts total_users=14 demo_users=4 total_requests=3 total_tenants=26 uat_tenants=0 total_instances=3. Portal http://100.76.217.35:8001/cloud/login 200, user1-4/123 login 302 to /cloud/setup 200, ports 8301-8304 FREE, live provisioning-worker Exited, UAT p3-uat-control-api Up, p3-uat-build-postgres healthy, isolated worker not running until Create click.
- **Evidence:** docs/reports/evidence/helpers-erp-cloud-manual-uat-v3/20260906T040329Z_55e870ce/ 118 files 12M 44 PNGs redacted, phase12/ 01_before_reset to 09_status_page, phase10/test_manifest.json, phase2_credential_rotation_proof.md, fail_closed_proof.md.
